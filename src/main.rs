...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        if (x_pos >= rl.get_screen_width() as f32 - x_size) {
            x_pos = rl.get_screen_width() as f32 - x_size;
            velocity_x = -velocity_x;
            current_hue = (current_hue + 30.0) % 360.0;
        }
        if (y_pos <= 0f32) {
        ...
        if (y_pos >= rl.get_screen_height() as f32 - y_size) {
            y_pos = rl.get_screen_height() as f32 - y_size;
            velocity_y = -velocity_y;
            current_hue = (current_hue + 30.0) % 360.0;
        }
